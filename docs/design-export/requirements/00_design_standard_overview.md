...
| Z升降+Barcode | 完整機構流程 + KEYENCE Barcode Reader(依BOM實際採購型號) | 可 Coding / 未實測 | 含定位氣缸、光罩在席、Z抬高、氣缸縮回、Barcode掃描位；通訊格式需實機確認 |
...
| 整機 Release | 現場驗收 | HOLD | Barcode Reader(依BOM實際採購型號)通訊、Z高度、Tolerance、Safety OFF、真空/煞車/門鎖Bypass需實測 |

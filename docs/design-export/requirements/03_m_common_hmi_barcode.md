...
| M1003 | 全機初始化完成 | Machine initialization complete | 共用流程 | Axis1+Axis2+Axis3初始化完成後ON | Axis4只手動，不作Auto Ready必要條件。 |
...
| M1128 | X橫移_Alarm旗標 | X traverse alarm | X橫移流程 | X橫移流程異常 |  |
...
| M1320 | Z升降_MC送電旗標 | Z lift MC power request | Z流程 | 控制Y0005 |  |
| M1321 | Z升降_開機完成旗標 | Z lift power ready | Z流程 | 伺服送電完成 |  |
| M1323 | Z升降_初始化請求旗標 | Z lift initialize request | Z流程 | 進S390/Axis3 OPR |  |
| M1324 | Z升降_初始化中旗標 | Z lift initializing | Z流程 | 初始化執行中 |  |
| M1325 | Z升降_初始化完成旗標 | Z lift initialized | Z流程 | Home/安全位完成 | Auto Ready使用M1325，不使用Axis4 M1425。 |
| M1326 | Z升降_Busy旗標 | Z lift busy | Z流程 | Z升降流程中 |  |
| M1327 | Z升降_Done旗標 | Z lift done | Z流程 | Z升降子流程完成 |  |
| M1328 | Z升降_Alarm旗標 | Z lift alarm | Z流程 | Z升降流程異常 |  |
| M1400 | Z旋轉_STL啟用旗標 | Z rotation STL enable | Z旋轉手動 | S400只允許工程/手動 | 自動禁用。 |
| M1500 | 三軸Auto伺服MC送電完成 | Auto 3-axis servo MC power completed | 軸卡共用 | Axis1+Axis2+Axis3伺服送電條件成立 | Axis4為手動調機，不作Auto必要。 |
...
| M6001 | HMI正向啟動_舊點位禁用 | Legacy forward start disabled | Deprecated | 不得作正式啟動 | 正式使用M6002。 |
| M6002 | HMI正向啟動 | HMI forward start | HMI自動 | Momentary，啟動M1102 | 儲存盒到上機盒。 |
| M6003 | HMI反向啟動 | HMI reverse start | HMI自動 | Momentary，啟動M1103 | 上機盒到儲存盒；Auto Busy中禁止。 |
...
| M6006 | Barcode生產前掃碼OK顯示 | Pre-production barcode OK display | HMI/Barcode | 只作M6301顯示鏡像，不作獨立Ready來源 | 避免與M6301雙重語意。 |
...
| M6031 | 人工料態套用確認 | Manual material state apply confirmed | HMI | PLC完成D6100人工修正後SET一掃描或短保持 |  |
...
| M6101 | X要求Z/Barcode流程 | X request Z barcode flow | X/Z交握 | X主流程要求Z升降+Barcode開始 |  |
| M6102 | Z流程已接收/Busy | Z flow acknowledge busy | X/Z交握 | Z流程開始後SET，完成或異常RST | 主流程確認Z已接手。 |
| M6103 | Z/Barcode流程完成且Z升降在安全位 | Z barcode done and Z lift safe | X/Z交握 | Z/Barcode成功完成且Z升降回安全位後SET | 不代表Z旋轉自動完成。 |
| M6104 | Service Unlock Request | Service unlock request | 工程/安全交握 | PLC/HMI只提出服務解鎖請求 | 不得作普通Safety Bypass；實際允許由安全硬體/安全控制器判定。 |
...
| M6202 | Barcode OK_舊點位禁用 | Legacy barcode OK disabled | Deprecated | 不得作正式比對OK | 正式使用M6304。 |
| M6203 | Barcode NG_舊點位禁用 | Legacy barcode NG disabled | Deprecated | 不得作正式比對NG | 正式使用M6305。 |
| M6210 | 手動命令_夾爪開 | Manual command gripper open | 手動單動 | HMI命令，PLC互鎖後驅動Y0007 | HMI不得直寫Y。 |
| M6211 | 手動命令_夾爪關 | Manual command gripper close | 手動單動 | HMI命令，PLC互鎖後驅動Y0010 | HMI不得直寫Y。 |
| M6212 | 手動命令_X手臂真空ON | Manual command arm vacuum on | 手動單動 | HMI命令，PLC互鎖後驅動Y0015 |  |
| M6213 | 手動命令_X手臂破真空 | Manual command arm vacuum release | 手動單動 | HMI命令，PLC互鎖後驅動Y0016 |  |
| M6214 | 手動命令_儲存盒真空ON | Manual command storage vacuum on | 手動單動 | HMI命令，PLC互鎖後驅動Y0017 |  |
| M6215 | 手動命令_儲存盒破真空 | Manual command storage vacuum release | 手動單動 | HMI命令，PLC互鎖後驅動Y0020 |  |
| M6216 | 手動命令_上機盒真空ON | Manual command load vacuum on | 手動單動 | HMI命令，PLC互鎖後驅動Y0021 |  |
| M6217 | 手動命令_上機盒破真空 | Manual command load vacuum release | 手動單動 | HMI命令，PLC互鎖後驅動Y0022 |  |
| M6220 | 手動命令_檢查站左右定位伸出 | Manual command LR locator extend | 手動單動 | PLC互鎖後驅動Y0011 | 與M6221互斥。 |
| M6221 | 手動命令_檢查站左右定位縮回 | Manual command LR locator retract | 手動單動 | PLC互鎖後驅動Y0012 | 與M6220互斥。 |
| M6222 | 手動命令_檢查站前後定位伸出 | Manual command FB locator extend | 手動單動 | PLC互鎖後驅動Y0013 | 與M6223互斥。 |
| M6223 | 手動命令_檢查站前後定位縮回 | Manual command FB locator retract | 手動單動 | PLC互鎖後驅動Y0014 | 與M6222互斥。 |
| M6250 | Axis1 Move Permit | Axis1 move permit | 手動互鎖 | X橫移允許 | Axis2上位+Z SafeForX+Safety OK。 |
| M6251 | Axis2 Move Permit | Axis2 move permit | 手動互鎖 | X升降允許 | X位置允許+Safety OK。 |
| M6252 | Axis3 Move Permit | Axis3 move permit | 手動互鎖 | Z升降允許 | X Clear + 定位汽缸狀態安全。 |
| M6253 | Axis4 Rotate Permit | Axis4 rotate permit | 手動互鎖 | Z旋轉允許 | X Clear + Z安全高度 + 定位汽缸縮回。 |
| M6300 | Barcode Enable | Barcode compare enable | 功能Enable | ON時Storage Barcode Valid必須成立才可Auto Start | OFF時Barcode結果記錄BYPASS。 |
| M6301 | Storage Barcode Valid | Storage barcode valid | Barcode Ready | 生產前工單/儲存盒Barcode有效 | Barcode Enable ON時Auto Ready必要。 |
| M6302 | Barcode讀取命令 | Barcode read command | Barcode | PLC觸發讀取 |  |
| M6303 | Barcode讀取完成 | Barcode read done | Barcode | Barcode Reader回傳完成 |  |
| M6304 | Barcode Compare OK | Barcode compare OK | Barcode | 正式比對OK | 取代M6202。 |
| M6305 | Barcode Compare NG | Barcode compare NG | Barcode | 正式比對NG | 取代M6203。 |
| M6306 | Barcode Bypass記錄 | Barcode bypass record | Barcode | Barcode Enable OFF時記錄Bypass |  |

...
| D4510 | 軸3_Z升降_Barcode掃描高度 | Axis3 barcode scan height | Z升降 | DINT | 到Barcode Reader(依BOM實際採購型號)掃描高度。 |
...
| D6300 | 工單標準Barcode字串起始 | Work order barcode string start | Barcode | STRING/WORD | Barcode Reader(依BOM實際採購型號)比對標準資料起始。 |
| D6350 | 光罩讀取Barcode字串起始 | Read barcode string start | Barcode | STRING/WORD | Barcode Reader(依BOM實際採購型號)讀取結果起始。 |
